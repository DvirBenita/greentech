...
/**
 * @file
 * Behaviors and functionality for hero blocks.
 */

((Drupal) => {
  /**
   * Set up a hero block in the current context.
   *
   * @type {Drupal~behavior}
   *
   * @prop {Drupal~behaviorAttach} attach
   *  Attaches the gtHero behavior to the right context.
   */
  Drupal.behaviors.gtHero = {
    attach(context) {
      const arrowElement = context.querySelector(".content-hint");
      if (arrowElement) {
        arrowElement.addEventListener("click", (event) => {
          ...
          window.scrollTo(
            0,
            window.scrollY +
              arrowElement.closest(".block-hero").getBoundingClientRect().bottom
          );
        });
      }
    },
