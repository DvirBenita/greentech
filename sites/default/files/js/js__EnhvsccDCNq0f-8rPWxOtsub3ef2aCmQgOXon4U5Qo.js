...
/**
 * @file
 * Behaviors and functionality for the Section Collection block.
 */
...
(($, Drupal) => {
   ...
   * @namespace
   */
  Drupal.csSectionCollection = Drupal.csSectionCollection || {
    /**
     * Options to pass into the intersection observer.
     *
     * Look just under the section collection header.
     *
     * @type {Array}
     */
    ...
    /**
     * How many pixels the visitor has scrolled over the header.
     *
     * @type {Integer}
     */
    scrollOver: null,
    ...
    /**
     * Update visibility classes as the observer intersects sections.
     *
     * @param {Array.IntersectionObserverEntry} entries
     *   A list of IntersectionObserverEntry objects.
     */
    handleIntersect(entries) {
      entries.forEach((entry) => {
        const section = entry.target;
        const sectionId = section.id;
        const sectionHeader = document.querySelector(`#${sectionId}-header`);

        if (entry.intersectionRatio > 0) {
          section.classList.add("visible");
          sectionHeader.classList.add("visible");
          sectionHeader.parentElement.classList.add("visible");
        } else {
          section.classList.remove("visible");
          sectionHeader.classList.remove("visible");
          sectionHeader.parentElement.classList.remove("visible");
        }
      });
    },

    /**
     * Initialize a Section Collection element.
     *
     * @param {element} element
     *   A Section Collection element.
     */
    initCollection(element) {
      // Set up the observer.
      const sections = element.querySelectorAll(
        ".section-collection-section-content[id]"
      );
      sections.forEach((section) =>
        Drupal.csSectionCollection.intersectionObserver.observe(section)
      );

      // Apply scrolling effects to the section collection header and top
      // toolbar.
      const topToolbar = document.querySelector("#app-bar");
      $(window).scroll(() => {
        const scrollOver =
          -1 *
          element
            .querySelector(".section-collection-section-header")
            .getBoundingClientRect().top;
        if (scrollOver > 0) {
          $(".section-collection-section-header img", element).css(
            "filter",
            `blur(${scrollOver / 15}px)`
          );

          // If our scrollOver value is the same as it was at the last scroll
          // event, then our header is collapsed and our toolbar is shown in
          // place of the regular top toolbar.
          if (scrollOver === this.scrollOver) {
            element.classList.add("header-collapsed");
            topToolbar.classList.add("visually-hidden");
          } else {
            element.classList.remove("header-collapsed");
            topToolbar.classList.remove("visually-hidden");
          }
        } else {
          $(".section-collection-section-header img", element).css(
            "filter",
    ...
    /**
     * Initialize all Section Collection elements in the given context.
     *
     * @param {HTMLDocument|HTMLElement} context
     *   An element to initialize section-collections within.
     */
    initAllCollections(context) {
      this.intersectionObserver = new IntersectionObserver(
        this.handleIntersect,
        this.intersectionObserverOptions
      );
      context
        .querySelectorAll(".section-collection:not(.activated)")
        .forEach(this.initCollection);
    },
  };

  /**
   * Set up Section Collection blocks in the current context.
   *
   * @type {Drupal~behavior}
   *
   * @prop {Drupal~behaviorAttach} attach
   *  Attaches the csSectionCollection behavior to the right context.
   */
  Drupal.behaviors.csSectionCollection = {
    attach(context) {
      // Initilize all Section Collections.
      Drupal.csSectionCollection.initAllCollections(context);
    },
  };
})(jQuery, Drupal);
;
